...
class InferSurrogate:
    """
    A class for searching and selecting neural network architectures for an ensemble
    using surrogate models to predict accuracy and diversity.
    """
    ...
    def run_selection(self, strategy: str = "greedy_forward") -> None:
        """
        The main method that triggers the selection process of models for the ensemble.

        Args:
            strategy (str): Model selection strategy.
        ...
        if strategy != "random":
            ...
            if not self.potential_archs:
                print(
                    "Error: No potential models found after filtering. Can't select an ensemble."
                )
                return
    ...
    def save_models(self, cleanup_tmp: bool = True) -> None:
        ...
        for i, arch in enumerate(self.selected_archs):
            ...
            file_name = (
                f"model_{model_id}.json"
                if model_id is not None
                else f"generated_model_{i+1}.json"
            )
    ...
    def greedy_forward_selection(
        ...
        """
        Greedy iterative selection (Forward Selection).
        1. Selects the first model with maximum accuracy.
        2. Iteratively adds models that maximize score (accuracy, diversity).
        """
        if len(archs) < n_select:
            print(
                f"Warning: Not enough candidates ({len(archs)}) to select {n_select}. Returning all candidates."
            )
            return archs
        ...
        candidate_accs = torch.cat(
            [candidate_accs[:best_first_idx], candidate_accs[best_first_idx + 1 :]]
        )
        candidate_embs = torch.cat(
            [candidate_embs[:best_first_idx], candidate_embs[best_first_idx + 1 :]]
        )
        ...
        while len(ensemble_archs) < n_select and len(candidate_archs) > 0:
            ...
            candidate_accs = torch.cat(
                [candidate_accs[:best_idx], candidate_accs[best_idx + 1 :]]
            )
            candidate_embs = torch.cat(
                [candidate_embs[:best_idx], candidate_embs[best_idx + 1 :]]
            )
    ...
    def _get_score(self, accuracy: torch.Tensor, dist: torch.Tensor) -> torch.Tensor:
        """
        Vectorized calculation of the "estimate" of the model.

                Args:
                    accuracy (torch.Tensor): A tensor with predicted accuracy (in percentages, 0-100).
        dist (torch.Tensor): A tensor with distances.

        Returns:
            torch.Tensor: A tensor with final scores.
        """
        gamma = self.config.acc_distance_gamma
        return (1 - gamma) * (accuracy / 100.0) + gamma * dist

    def _clear_all_buffers(self) -> None:
        """Clears all temporary lists with models in `self`."""
    ...
    def paint_tsne(self, X, centroids, cluster_ids):
        ...
        # plt.title("t-SNE projection of embeddings after PCA-50")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
